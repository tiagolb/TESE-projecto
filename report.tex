\documentclass{./llncs2e/llncs}
\usepackage{graphicx}
\usepackage{float}
\usepackage{mathtools}
\usepackage[utf8]{inputenc}
\usepackage[nolist,nohyperlinks]{acronym}
% Maintain images and tables within their respective sections
\usepackage[section]{placeins}
%\usepackage{tikz}
%\usepackage{pgfgantt}
%\usepackage{gantt}

\usepackage{array}
\newcolumntype{P}[1]{>{\centering\arraybackslash}p{#1}}

\usepackage{multirow}

\usepackage{pifont}% http://ctan.org/pkg/pifont
\newcommand{\cmark}{\ding{51}}%
\newcommand{\xmark}{\ding{55}}%

\renewcommand{\figurename}{Figure}

%\usepackage{refcheck}
%
% Change the margins
%
% \usepackage[margin=2.9cm]{geometry}

\begin{document}
\title{Dissertation: ViewZone}

\subtitle{Embedding Secure Displays in Insecure Mobile Interfaces Using ARM TrustZone}
\author{Tiago Luís de Oliveira Brito\\
Supervisor: Prof. Nuno Santos}

\institute{Instituto Superior Técnico,\\
Av. Rovisco Pais, 1049-001 Lisboa - PORTUGAL\\
\textbf{tiago.de.oliveira.brito@tecnico.ulisboa.pt}}

\maketitle

% make a proper TOC despite llncs
%\setcounter{tocdepth}{2}
\setcounter{secnumdepth}{3}
\setcounter{tocdepth}{3}
\makeatletter
\renewcommand*\l@author[2]{}
\renewcommand*\l@title[2]{}
\makeatletter
\renewcommand\subsubsection{\@startsection{subsubsection}{2}{\z@}%
	{-18\p@ \@plus -4\p@ \@minus -4\p@}%
	{8\p@ \@plus 4\p@ \@minus 4\p@}%
	{\normalfont\normalsize\bfseries\boldmath
		\rightskip=\z@ \@plus 8em\pretolerance=10000 }}

%\cleardoublepage
\input{sections/1-abstract.tex}
%\input{sections/2-keywords.tex}

%\cleardoublepage
%\tableofcontents
%\cleardoublepage
\pagestyle{plain}
\pagenumbering{arabic}
\input{sections/3-introduction.tex}
%\cleardoublepage
%\input{sections/4-objectives.tex}
%\cleardoublepage
\input{sections/5-related-work.tex}
%\cleardoublepage
\input{sections/6-architecture.tex}
%\cleardoublepage
\input{sections/7-evaluation.tex}
%\cleardoublepage
\input{sections/8-conclusions.tex}
%\cleardoublepage

%[PREVIOUS/ORIGINAL CONFIG]
%\newpage
%\appendix
%\input{sections/9-appendix.tex}
%\tableofcontents
%\cleardoublepage

%
% Bibliography
%
%\Urlmuskip=0mu plus 1mu
\bibliographystyle{plain}
\addcontentsline{toc}{section}{References}
\bibliography{references}

%\newpage
%\appendix
%\input{sections/9-appendix.tex}
%\tableofcontents

% *** DEFINITION OF ACRONYMS ***
\acrodef{PC}{Personal Computer}
\acrodef{OS}{Operating System}
\acrodef{TCB}{Trusted Computing Base}
\acrodef{LOC}{Lines Of Code}
\acrodef{CNN}{Cable News Network}
\acrodef{HIPAA}{Health Insurance Portability and Accountability Act}
\acrodef{API}{Application Programming Interface}
\acrodef{DRM}{Digital Rights Management}
\acrodef{DMB}{external-device misbonding}
\acrodef{PHR}{Personal Health Record}
\acrodef{OMA}{Open Mobile Alliance}
\acrodef{DAC}{Discretionary Access Control}
\acrodef{MAC}{Mandatory Access Control}
\acrodef{ASM}{Android Security Modules}
\acrodef{ASF}{Android Security Framework}
\acrodef{SoC}{Socket-on-Chip}
\acrodef{PIN}{Personal Identification Number}
\acrodef{MMU}{Memory Management Unit}
\acrodef{GPUs}{Graphics Processing Units}
\acrodef{IMEI}{International Mobile Equipment Identity}
\acrodef{IPC}{Inter Process Communication}
\acrodef{TEE}{Trusted Execution Environment}
\acrodef{GPFR}{General-purpose Frameworks and Runtimes}
\acrodef{TA}{Trusted Applications}
\acrodef{DPM}{Data Processing Module}
\acrodef{NMI}{Non-maskable Interrupts}

\end{document}
