...
\begin{figure}[t!]
	...
	\label{fig:relwork}
\end{figure}
%Podes chamar a isto Trusted Execution System
...
% se calhar tirar o exemplo da physical security para não confundir o leitor
...
%Atenção que isto não está inteiramente correcto. Estuda melhor a diferenca entre MAC e DAC. DAC quer dizer que o acesso a um objecto é totalmente determinado pelo dono do objecto. Por exemplo, no Linux os utilizadores podem livremente mudar as permissões dos seus ficheiros. Com MAC já não é assim. Por exemplo, com SELinux há restrições a mudança de permissões que podem ser feitas pelo dono do recurso. Essas restrições são definidas por uma política de segurança global. Sugiro que leias mais sobre DAC e MAC em Android na net. Vê na pg. 8: http://www.zhauniarovich.com/files/asani/asani.pdf
...
% clarificar porque é que o MAC é robusto
...
% acho que também é o problema de uma aplicação poder solicitar mais permissões do que aquelas que ela efectivamente precisa. Isso é também uma motivação para permission refinement.
...
Several context-aware permission refinement systems exist. Trusted third parties can use CRePE~\cite{conti2011crepe} to enforce security policies on another devices whilst, for example, the employees' mobile devices are inside the company, but have no privileges otherwise. % melhorar ingles desta frase
...
%Está complexo, simplifica esta ultima frase
...
%Refere estas frameworks mais cedo no texto assim que fales de frameworks no parágrafo anterior.

\paragraph{\textbf{Memory Instrumentation}}
%liga melhor esta transicção: qual é a relação de memory instrumentation com access control mechanisms? Pelo que eu percebo a ideia é refinar o controlo de acesso às aplicações através de técnicas de análise de código da aplicação.

Memory instrumentation can be divided in two groups: static memory instrumentation and dynamic memory instrumentation. While static memory instrumentation changes already compiled bytecode, dynamic instrumentation patches running processes. Dynamic memory instrumentation is an interesting mechanism as it supports the enforcement of new security models. DeepDroid~\cite{wang2015deepdroid} relies on dynamic memory instrumentation to enforce fine-grained context-aware security policies for enterprise.%Esta ideia é a mais importante para motivar memory instrumentation. Coloca no início do parágrafo.
...
%Isto não resolve toda a nossa vida? (nao tem secure IO)
...
% Qual é o primeiro problematic mechanism? Tens que clarificar isto pois o leitor já não se lembra.
...
% Explica melhor isto com base na figura (a).
...
% Melhora a transição da frase anterior para esta para se perceber melhor que o Saint tem como objectivo mitigar este tipo de ataques.
...
The research community has been experimenting with trusted execution environments in order to achieve better solutions for common security problems, such as two-factor authentication or cryptographic key generation.
% Aqui não deves referir-te a TEE pois TEE é apenas um caso particular de um tipo de sistemas que podes construir com trustzone. Eu entraria logo a dizer que a tecnologia ARM TrustZone é um tipo de trusted hardware que permite o desenvolvimento de um conjunto variado de serviços / sistemas de segurança, como por exemplo X ou Y. Dizer que um dos aspectos chaves desta technologia tem a ver com o facto de permitir execução de código em isolamento do sistema operativo e portanto permitir reduzir a trusted computing base do sistema.
...
% evita este tipo de adjectivos "propagandísticos"
...
% Para iniciar este parágrafo, podes dizer que um dos usos mais importantes de TZ é para suportar o desenvolivmento de trusted execution environments (TEE), e depois explicar.
...
When surveying the literature, TrustZone-based systems can be divided in two separate groups: \ac{GPFR}, and special-purpose \ac{TA}. \ac{GPFR} represent tool kits for building secure special-purpose operating systems or runtimes, which allow the development of Trusted Applications. Special-purpose \ac{TA} are secure applications, built on top of \ac{GPFR} systems, which are executed in the trusted execution environment to achieve a specific security property. With a more thorough analysis, one can divide each of these categories even further, with regards to trusted user interface (UI). 
% Eu não usaria o termo TEE feature, pois pode não ser associado exclusivamente a TEE. Podes dizer que a TrustZone-based system features Trusted UI capability if it allows secure world components to directly access the mobile interface without interference from the rich OS...
...
% Gosto do facto de teres atribuido nomes específicos, mas talvez simplificar um pouco as designações e os conceitos, por exemplo Trusted Kernels (TK), e Trusted Services (TS). Um TK permite a execução código genérico no mundo seguro, um TS implementa uma aplicação específica no mundo seguro.
...
% Coloca a Tabela 3 nesta página.

% ESTES NOMES ESTAO ESQUISITOS, PERGUNTAR AO PROF POR OUTRO NOME
\subsubsection{GPFR with Untrusted UI}

% Começas muito abruptamente. Tens que explicar novamente o que é GPFR para depois avançares.
...
% above, i.e., explica. Adiciona também uma ou duas frases que relacionem com o teu projecto: porque é que estes sistemas não resolvem o problema que te propoes resolver?

\subsubsection{TA with Untrusted UI}

% Liga melhor com a secção anterior, comparando algum aspecto com os sistemas que introduziste na secção anterior. Por exemplo, "As opposed to GPFR, which enable the execution of general-purpose application code on the secure world, TA are designed to implement specific applications from the secure world natively."
...
% Isto é mesmo verdade? Alguns dos papers como o Droidvault não correm em cima de nenhum runtime, mas correm nativamente no secure world... ou está a falhar-me alguma coisa?
...
% Não percebi qual é a funcionalidade que é oferecida por esta aplicação? Neste paper, o sistema tem algum nome?
%COMO SE PODE VER NAO PRECISA DE SECURE UI, dar o exemplo do paper(da figura)?

Other trusted applications are built using generic runtimes and frameworks. DroidVault~\cite{li2014droidvault} introduces the notion of data vault as a protective measure for sensitive data on Android mobile devices. % vago... explica melhor para que serve isto, o que é um data vault?
...
% Falta uma ou duas frases que relacionem com os objectivos do teu projecto. Isto resolve o teu problema? Porquê?
...
%Também aqui, no final, tens que relacionar com o objectivo do teu projecto. Porque é que estes sistemas não resolvem o teu problema original?
...
% Acho que a transição pode ser melhorada, relacionando melhor com a última secção. Depois no parágrafo de abertura, introduziria a questão dizendo que têm sido desenvolvidas aplicações bastante diversificadas para correrem no mundo seguro e que oferecem uma trusted UI.
...
% melhorar esta frase porque a maior parte nao implementa as drivers, mas para conseguir ter trusted ui essa implementação é a escolha mais comum
...
% o que são estas primitivas e o que têm a ver com trustzone?
...
% O KNOX não pertence mais à categoria anterior? Não pode ser configurado para correr aplicações arbitrárias?
...
% Isto é o nosso concorrente directo. Explica mais calmamente.
% Parte isto em mais parágrafos e se achares que ajuda, coloca aqui uma figura do sistema.

% No final desta secção, tem que ser muito claro porque é que nenhum destes sistemas resolve o problema que queremos resolver. Em especial o TrustUI: porque é que não é suficiente para nós?
...
\end{table}
% Coloca este quadro lá em cima, no início da secção 2.3.

\paragraph{Summary} Table~\ref{tab:trustzonesystemscomparison} briefly summarizes the categorization of systems described in this section according to the partitioning documented above. % NOSSO SISTEMA DEVE ENCAIXAR EM QUE CELULA?
