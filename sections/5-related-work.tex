...
Most of the \ac{TEE} software stack implementations have similar architectures (similar to the one shown in section~\ref{sec:tee}). This architecture is generally composed of a small trusted kernel running in the isolated secure world of TrustZone-enabled processors, a normal world user space client API and a kernel TEE device driver used to communicate between worlds. OP-TEE~\footnote{https://wiki.linaro.org/WorkingGroups/Security/OP-TEE} and AndixOS~\cite{fitzekandix} are \ac{TEE} implementations which share this general architecture. Although these systems use TrustZone's hardware based isolation to ensure that applications running inside the secure world are not tempered by a compromised rich OS, there are some limitations regarding the applications it can support, this is because these systems were implemented %TCB

ObC~\cite{kostiainen2012board} Originalmente nao foi para trustzone, no entanto foi posteriormente adaptada

TLR~\cite{santos2011trusted}

\subsubsection{TA with Untrusted UI}

Android Key Store~\footnote{http://developer.android.com/training/articles/keystore.html} The encryption/decryption of the keystore is handled by the keystore service, which in turn links with a Hardware Abstraction Layer module called "keymaster". The AOSP provides a sofware ("softkeymaster") implementation, but device vendors can offer support for hardware based protected storage, if available. On ARM architectures, this linked to the ARM "TrustZone".

DroidVault~\cite{li2014droidvault}

Brasser et al.~\cite{brasserregulating}

Samsung KNOX~\cite{knox_whitepaper} isto devia ter secure UI mas nao vi isso explicito e nao esquecer de mudar isto para um cite de technical paper.

\subsubsection{GPFR with Trusted UI}

SierraTEE~\footnote{http://www.openvirtualization.org/}

Genode~\footnote{http://genode.org/}

T6~\footnote{https://www.trustkernel.com/products/tee/t6.html}

Open-TEE~\cite{mcgillion2015open} diz que cumpre as especificações de device da globalPlatform que inclui trusted UI
Trusted UI a ser extendido pela comunidade

TrustICE~\cite{sun2015trustice}
...
\begin{table}[t!]
	...
	\begin{tabular}{ c|c|c| }
		...
		\multirow{4}{*}{TAs} 	& Android Key Store						& TrustOTP \\
								& DroidVault 							& TrustDump \\
								& Restricted Spaces ****				& AdAttester \\
								& 			 							& TrustUI \\
								\cline{2-3}
		\multirow{8}{*}{GPFR}	& TLK									& Genode \\
								& OP-TEE 								& T6 \\
								& Mobicore								& TrustICE \\
								& Samsung Knox							& SierraTEE \\
								& TLR 									& \\
								& Andix OS 								& \\
								& Nokia ObC								& \\
		\cline{2-3}
