...
Mobile devices are growing and being adopted everywhere. Because of this mobile device ubiquity, critical mobile applications that require very sensitive data to be displayed are proliferating, such as mHealth and e-money applications. But, in order to display sensitive information to the user in most modern mobile platforms, applications must rely not only in its own code but the code of all the underlying layers, which include a full-featured operating system. These rich operating systems have, generally, a very large and complex code base, which makes it difficult to assess its correctness and ensure security guarantees to critical mobile applications. This paper proposes and implements a solution for generic applications to securely output sensitive data without the need to rely on a native full-featured operating system, but rather on a reduced TCB built using ARM's secure hardware technology, TrustZone. To motivate and prove the concept of this solution, a critical mobile health application will be developed.
